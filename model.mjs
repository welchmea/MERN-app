...
mongoose.connect(
  process.env.MONGODB_URI || process.env.MONGODB_CONNECT_STRING,
...
vitamin_db.once("open", (err) => {
  if (err) {
    ...
    console.log(
      "You have successfully connected to MongoDB Vitamins collection using Mongoose."
    );
  }
});
...
const updateVitamin = async (_id, name, date, purpose, unit_day) => {
  const result = await Vitamin.replaceOne(
    { _id: _id },
    {
      name: name,
      date: date,
